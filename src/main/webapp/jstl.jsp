...
<html>
	...
	<body>
		<c:set var="to"   value="10"/> <!-- to 라는 변수에 10을 저장한 것! -->
		<c:set var="arr"  value="10,20,30,40,50,60,70"/> <!-- 배열은 이렇게 만듬! -->
		
		<c:forEach var="i" begin="1" end="${to}"> <!-- jstl에서의 for문. 위에서 선언한 변수는 el태그로! -->
			${i}
		</c:forEach>
		
		<br>
		
		<c:if test="${not empty arr}"> <!-- jstl에서의 if문. test에 조건문을 적어준다. -->
